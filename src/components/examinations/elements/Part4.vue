  ...
  methods: {
    ...
    async addQuestion() {
      if (this.questionCount < 30) {
        this.questions.push(
          Object.assign({ no: null, part: 4 }, this.questionDataSeed)
        );
      }
    },
    addExample() {
      this.questions.push(
        Object.assign({ no: 0, part: 4 }, this.questionDataSeed)
      );
    },
    getPart1() {
      request({
        url:
          "/backend/examinations/questions/" + this.$route.params.code + "/4",
        method: "get"
      })
        .then(res => {
          console.log(res.data.result_data);
