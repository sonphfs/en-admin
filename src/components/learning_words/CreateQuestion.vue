        ...
        <div class="modal-content">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-9 col-sm-9 col-xs-12">
                    ...
                    <audio controls controlslist="nodownload" v-if="question.audio">
                      <source :src="'http://127.0.0.1:8001/'+ question.audio" type="audio/mpeg" />
                    </audio>
                  </div>
                </div>
              </form>
          ...
          <div class="modal-footer">
            <button
              ...
              @click="close()"
            >Cancel</button>
            <button type="button" class="btn btn-danger antosubmit" @click="createOrUpdateQuestion()">{{ action }}</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    createOrUpdateQuestion(){
      this.question.word_id = this.learningWordSelected.id
      let data = {
        "question": this.question
      }
      request({
        url: "/backend/questions/create-or-update",
        method: "post",
        data
      }).then(res=> {
        // this.close()
