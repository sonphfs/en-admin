<template>
  <div class="col-md-12 col-sm-12 col-xs-12">
    ...
    <CreateQuestion v-if="modalOpen==true" @onClose="modalOpen=false"></CreateQuestion>
  </div>
</template>
<script>
...
import CreateQuestion from "@/components/learning_words/CreateQuestion";
...
export default {
  ...
  methods: {
    ...
    getQuestions(page = 1) {
      let baseUrl = "/backend/questions/list?page=" + page + "&type=word"
      if(this.learningWordSelected != null) {
        baseUrl += "&word_id=" + this.learningWordSelected.id
      }
      if(this.keyword != "") {
        baseUrl += "&keyword=" + this.keyword
      }
      request({
        url: baseUrl,
        method: "get"
